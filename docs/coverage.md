...
| 常用机器设置与拨杆模式 | MachineSettingChange、SettingsWriteTracker、MachineSettingsActivity、HomeActivity | 萃取/蒸汽温度、两路加热、照明、自动待机时间、手动/自动压力/自动流量共86条允许参数报文与旧版编码函数对照；写入后以新鲜匹配0x83回读确认 | 尚未实机验证设置回读时序；运行模式位0x04及睡眠、待机温度、温差补偿等写入未开放 |
...
Alpha 日常版第一段功能见[说明](mobile-alpha.md)。自动待机时间接入后，协议检查35,180项、会话检查34项、Alpha单测30项通过，debug构建成功，Lint 0错误、2条警告。当前用户要求暂不安装，尚无Alpha实机萃取、槽位启停、独立去皮或设置回读证据。Lab独占连接约6分钟、后台/锁屏各约1分钟持续收数，见上方报告。
