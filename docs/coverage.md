...
| 旧版历史显式导入 | LegacyHistoryCodec/Store、LegacyHistoryActivity | 按旧版 `brew_history_v1` 保存结构构造 JSON；严格校验、独立存储、冲突拒绝、重复导入与图表单位说明；Alpha/Mock 单测各72项通过 | 缺少旧包真实导出文件与导出入口；未实机验证 SAF 和旧数据兼容 |
...
Alpha 日常版第一段功能见[说明](mobile-alpha.md)。历史与曲线导出接入后，协议检查36,796项、会话检查36项、Alpha与Mock各72项单测通过，双变体构建成功，Lint 0错误、2条警告。当前用户要求暂不安装，尚无Alpha实机萃取、槽位启停、独立去皮、睡眠、睡眠计划写入、设置回读或告警证据。Lab独占连接约6分钟、后台/锁屏各约1分钟持续收数，见上方报告。
