...
详见 `docs/validation-2026-09-23.md`。独立APK已安装；原生咖啡机认证/设置与遥测收到，固件1.1.3。新增仪器测试实机通过：未知数据、Activity重建、前后台服务保持、停止服务后进程内ZIP导出。真实BLE出现两次status=8断线，尚未定位；第二次早于切后台24秒。08:42已实机双连，BOOKOO四条初始化+Ready及约2分钟并行遥测确认。08:44运行仪器测试重启进程，中断当时连接；后续必须用`scripts/run_lab_smoke.sh`先检查服务不存在。秤负重量/自动重连、SAF文件选择器全流程、长期BLE和萃取尚未验收。新增认证未确认时只接收遥测也不Ready的回归，会话场景增至33。
