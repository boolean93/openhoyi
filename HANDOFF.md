...
`mobile` 第一段原生日常版已完成首页连接、实时状态、传输日志导出、三条采集曲线浏览/选择和萃取页；见 `docs/mobile-alpha.md`。萃取开始需显式确认，服务层要求咖啡机新鲜待机数据及已验证曲线，重量模式要求新鲜秤数据；运行中拦截主动断链和停服务。`trace-core`从Lab抽取后，两款App的单测、构建、Lint均通过。平板两次拒绝安装新包（`INSTALL_FAILED_USER_RESTRICTED`），因此尚无Alpha的实机启动或控制证据；APK位于`mobile/build/outputs/apk/debug/mobile-debug.apk`。不能把代码验收当成硬件等效或完整替代App。
