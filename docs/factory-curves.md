...
旧版连接电子秤时，最大水量按目标重量扩大：`max(flowMl, ceil(4 × weightGram))`；无秤时使用曲线水量，且不按重量停止。当前实现分别验证这两种报文；进入启动确认时展示模式、目标重量和最大水量。报文等同旧版不代表机器实测等同，工厂曲线仍待人工监督下的实机验收。旧版 `hy_chartLib` 可从修改版显式导出并在 Alpha 独立只读浏览；详情见[迁移边界](legacy-curve-import.md)。用户曲线的控制和曲线编辑未完成；旧版私有数据不能从不同包名的 Alpha 直接读取。
