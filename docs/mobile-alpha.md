...
`./gradlew :protocol-core:check :device-session:check :app:testDebugUnitTest :app:assembleDebug :app:lintDebug :mobile:testDebugUnitTest :mobile:assembleDebug :mobile:lintDebug -PgoogleMirror=aliyun -Dhttp.proxyHost= -Dhttps.proxyHost=` 已通过。曲线单测核对三条20字节启动帧、目标重量及唯一ID；Lab日志测试在抽取共享模块后仍通过。2026-09-23两次实机安装均被平板返回 `INSTALL_FAILED_USER_RESTRICTED`，因此不能声称 Alpha 的 UI 或设备连接已实机通过。Lab 的实机证据不自动等同于 Alpha 的实机验收。
